...
- **`src/laws/hierarchy.ts`** also exports `canonicalNormaId()` and `suggestNormaId()` — the analogue of `normalizeNumber` for norma identifiers. `canonicalNormaId` does lossless normalization (case, diacritics, whitespace/dash/dot/comma separators, intra-numeric punctuation) and returns a canonical key only if it lands on one in `TIER_BY_NORMA_ID`. `suggestNormaId` runs when canonicalization fails; it returns a single candidate when the input's digit run or alphanumeric form uniquely matches one canonical id (≥4 chars threshold; null when 0 or ≥2 matches). Both are pure: the SQLite repo canonicalizes on entry to every method that filters by `norma_id`, and `server.ts` consults the suggester to enrich NOT_AVAILABLE payloads with `¿Quisiste decir \`<id>\`?` and `structuredContent.suggestion`. The repo also has a private `resolveNormaId()` cascade — `canonicalNormaId` → `findNormaByShortName` (exact match on `nombre_corto`, e.g. `LNPA` → `ley_19549`) → raw — so unique short-name aliases auto-resolve without a roundtrip.
