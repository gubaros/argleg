...
const LDC_ARTICLE_RE =
  /^\s*ART[IÍ]CULO\s+(\d+\s*[°º]?\s*(?:bis|ter|quater|quinquies|sexies)?|\d+)\s*[\-–—:.]+\s*/gimu;

export function parseLey24240(html: string): Article[] {
  let text = htmlToText(html);
  const first = /ART[IÍ]CULO\s+1\s*[°º]?\s*[\-–—:.]+\s*Objeto/iu.exec(text);
  if (!first) return [];
  ...
  const re = new RegExp(LDC_ARTICLE_RE.source, LDC_ARTICLE_RE.flags);
